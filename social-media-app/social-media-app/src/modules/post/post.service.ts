...
private realTime:typeof realTimeGateway
...
// normalize createdBy to ObjectId (handles populated user or raw ObjectId)
const recipientId = (post.createdBy as any)?._id ?? post.createdBy
const socketIds = await this.redis.getSockets(recipientId)
if(socketIds.length){
  this.realTime.getIo().to(socketIds).emit("likePost",{
    message: `${user.firstName} reacted to your post`,
    postId,
    userId:user._id,
    react: Number(react)
